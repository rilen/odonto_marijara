// client/src/components/Contatos.jsx
...
const Contatos = () => {
  ...
  const [contactToDeleteId, setContactToDeleteId] = useState(null); // ID do contato a ser excluído
  ...
  const fetchContatos = async () => {
    try {
      ...
      if (response.ok) {
        // Para cada contato, buscar o status financeiro se for paciente
  ...
  const fetchPatientFinancialStatus = async (pacienteId) => {
    try {
      ...
      if (response.ok && data.status) {
        return data.status; // Retorna 'Pago', 'Atrasado', 'Nao Pago', 'N/A'
      } else {
        return 'N/A';
      }
    } catch (err) {
  ...
  const handleConfirmDelete = async () => {
    ...
    if (contactToDeleteId) {
      try {
        const response = await fetch(`/api/contatos/${contactToDeleteId}`, { method: 'DELETE' });
        if (response.ok) {
          fetchContatos();
          setError('');
        } else {
        ...
        setContactToDeleteId(null); // Limpa o ID do contato a ser excluído
      }
    }
  };
                    ...
                    <tr key={contato.id} className="border-b border-gray-200 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-6 text-center">
                        <div className="flex item-center justify-center">
                          ...
                          <button
                            onClick={() => handleExcluirClick(contato.id)} // Usa o novo handler para exclusão
                          ...
                          {contato.tipo === 'Paciente' && (
                            <Link
                              to={`/anamnese/${contato.id}`} // Link para o formulário de Anamnese com o ID do paciente
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full text-center">
            ...
            <p className="mb-6">Tem certeza que deseja excluir este contato?</p>
